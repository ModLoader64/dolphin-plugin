...
{
  return Core::IsRunning(Core::GetSystem());
}

EXPORT bool core_is_running_and_started()
{
  return Core::IsRunningOrStarting(Core::GetSystem());
}
...
{
  return core_is_running() && core_is_cpu_thread();
}
...
{
  return static_cast<int>(Core::GetState(Core::GetSystem()));
}
...
{
  Core::RunOnCPUThread(Core::GetSystem(), function, wait_for_completion);
}
...
EXPORT void core_queue_host_job(std::function<void(Core::System&)> job, bool run_during_stop)
{
...
{
  Core::HostDispatchJobs(Core::GetSystem());
}
